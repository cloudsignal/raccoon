...
import { agentAddress, createEnvelope, userAddress } from '@raccoon/protocol';
...
describe('chatReducer', () => {
  ...
  it('bumps unread for agent messages caught up on a background reconnect, not while viewing (#4/#10)', () => {
    ...
    let state = chatReducer(emptyChatState, {
      type: 'history', convKey: 'coordinator', agentId: 'coordinator',
  ...
  it('appends live agent messages, dedupes, clears typing, counts unread when inactive', () => {
    ...
    state = chatReducer(state, { type: 'message', env, convKey: env.channel, active: false });
    state = chatReducer(state, { type: 'message', env, convKey: env.channel, active: false });
    expect(state.messages['coordinator']).toHaveLength(1);
    expect(state.typing['coordinator']).toBe(false);
    expect(state.unread['coordinator']).toBe(1);
    state = chatReducer(state, { type: 'read-channel', convKey: 'coordinator' });
    expect(state.unread['coordinator']).toBe(0);
  });

  it('a redelivered agent message (dedupe no-op) still clears a live typing indicator', () => {
    const env = agentMsg('hello');
    let state = chatReducer(emptyChatState, { type: 'message', env, convKey: env.channel, active: false });
    ...
    state = chatReducer(state, { type: 'typing', convKey: 'coordinator', on: true });
  ...
  it('a redelivered approval request (dedupe no-op) still clears typing', () => {
    ...
    let state = chatReducer(emptyChatState, { type: 'approval', env, convKey: env.channel, active: true });
    state = chatReducer(state, { type: 'typing', convKey: 'coordinator', on: true });
    state = chatReducer(state, { type: 'approval', env, convKey: env.channel, active: true });
    expect(state.typing['coordinator']).toBe(false);
    expect(state.messages['coordinator']).toHaveLength(1);
  });

  it('typing action is a state no-op when the flag is unchanged', () => {
    const s1 = chatReducer(emptyChatState, { type: 'typing', convKey: 'coordinator', on: false });
    expect(s1).toBe(emptyChatState);
    const s2 = chatReducer(s1, { type: 'typing', convKey: 'coordinator', on: true });
    const s3 = chatReducer(s2, { type: 'typing', convKey: 'coordinator', on: true });
    expect(s3).toBe(s2);
  });
  ...
  it('tracks optimistic sends through ack to delivery', () => {
    ...
    state = chatReducer(state, { type: 'ack', convKey: 'coordinator', refId: 'm1', status: 'received' });
    expect(state.messages['coordinator']![0]!.delivery).toBe('sent');
    state = chatReducer(state, { type: 'ack', convKey: 'coordinator', refId: 'm1', status: 'read' });
  ...
  it('stores approval requests as approval messages and records responses', () => {
    ...
    expect(state.unread['coordinator'] ?? 0).toBe(0);
    state = chatReducer(state, { type: 'responded', convKey: 'coordinator', refId: 'task-9', choice: 'approve', responseId: 'resp-1' });
  ...
  it('reconcile-responses rehydrates a failed response onto a history-loaded approval, but never clobbers live state (#R7-2)', () => {
    ...
    let state = chatReducer(emptyChatState, { type: 'approval', env, convKey: env.channel, active: false });
  ...
  it('tracks approval-response delivery via responseEnvId through ack and failure (#R2-5)', () => {
    ...
    state = chatReducer(state, { type: 'ack', convKey: 'coordinator', refId: 'resp-1', status: 'received' });
    ...
    state = chatReducer(state, { type: 'delivery', convKey: 'coordinator', id: 'resp-1', delivery: 'failed' });
  ...
  it('a stale received ack does not un-fail a failed response, but a real delivered still recovers it (#R8-2)', () => {
    ...
    state = chatReducer(state, { type: 'ack', convKey: 'coordinator', refId: 'resp-1', status: 'failed' });
    expect(state.messages['coordinator']![0]!.respondedDelivery).toBe('failed');
    ...
    state = chatReducer(state, { type: 'ack', convKey: 'coordinator', refId: 'resp-1', status: 'delivered' });
  ...
  it('a stalled ack is terminal-non-retryable: a stale received cannot un-stall it, delivered still recovers (#P1-A)', () => {
    ...
    let state = chatReducer(emptyChatState, { type: 'approval', env, convKey: env.channel, active: true });
    state = chatReducer(state, { type: 'responded', convKey: 'coordinator', refId: 'task-9', choice: 'approve', responseId: 'resp-1' });
    state = chatReducer(state, { type: 'ack', convKey: 'coordinator', refId: 'resp-1', status: 'stalled' });
  ...
  it('a late timeout-failure regresses delivered via the UNGATED delivery path but not via the monotonic ack path (#P1-E4)', () => {
    ...
    let base = chatReducer(emptyChatState, { type: 'approval', env, convKey: env.channel, active: true });
    base = chatReducer(base, { type: 'responded', convKey: 'coordinator', refId: 'task-9', choice: 'approve', responseId: 'resp-1' });
    base = chatReducer(base, { type: 'ack', convKey: 'coordinator', refId: 'resp-1', status: 'delivered' });
    ...
    const viaDelivery = chatReducer(base, { type: 'delivery', convKey: 'coordinator', id: 'resp-1', delivery: 'failed' });
    ...
    const viaAck = chatReducer(base, { type: 'ack', convKey: 'coordinator', refId: 'resp-1', status: 'failed' });
  ...
  it('retry (delivery reset) re-opens the monotonic gate so a fresh received advances again (#R8-2)', () => {
    ...
    state = chatReducer(state, { type: 'ack', convKey: 'coordinator', refId: id, status: 'failed' });
    expect(state.messages['coordinator']![0]!.delivery).toBe('failed');
    // User retry uses the 'delivery' action (ungated) to reset to pending...
    state = chatReducer(state, { type: 'delivery', convKey: 'coordinator', id, delivery: 'pending' });
    expect(state.messages['coordinator']![0]!.delivery).toBe('pending');
    // ...and the resend's fresh 'received' legitimately advances pending→sent.
    state = chatReducer(state, { type: 'ack', convKey: 'coordinator', refId: id, status: 'received' });
    expect(state.messages['coordinator']![0]!.delivery).toBe('sent');
  });
  ...
  it('reconcile-approvals replaces a history text row with the interactive card (#R8-1)', () => {
    ...
    state = chatReducer(state, { type: 'reconcile-approvals', convKey: 'coordinator', approvals: [env] });
    const m = state.messages['coordinator']![0]!;
  ...
  it('sorts merged history + live by ts', () => {
    let state = chatReducer(emptyChatState, { type: 'message', env: agentMsg('live'), convKey: 'coordinator', active: true });
    state = chatReducer(state, {
      type: 'history', convKey: 'coordinator', agentId: 'coordinator',
...
describe('multi-pairing key domain', () => {
  const A = '01ARZ3NDEKTSV4RRFFQ69G5FAV/coordinator';
  const B = '01BX5ZZKBKACTAV9WEVGEMMVRZ/coordinator';

  it('two pairings exposing the same channel name keep separate messages, unread, and typing', () => {
    let state = emptyChatState;
    state = chatReducer(state, {
      type: 'history', convKey: A, agentId: 'coordinator',
      messages: [{ id: 'm1', role: 'agent', text: 'from A', ts: '2026-08-01T00:00:01.000Z' }],
      active: false,
    });
    ...
    expect(state.messages[A]!.map((m) => m.text)).toEqual(['from A']);
    expect(state.messages[B]!.map((m) => m.text)).toEqual(['from B']);
    expect(state.unread[B]).toBe(1);
    expect(state.unread[A] ?? 0).toBe(0);
    expect(state.typing[B]).toBe(true);
    expect(state.typing[A] ?? false).toBe(false);
  });

  it('read-channel on one pairing leaves the other pairing untouched', () => {
    let state = emptyChatState;
    const env = createEnvelope('msg', {
      from: agentAddress('coordinator'), to: userAddress('u1'), channel: 'coordinator',
      payload: { text: 'x' },
    });
    state = chatReducer(state, { type: 'message', env, convKey: A, active: false });
    const envB = createEnvelope('msg', {
      from: agentAddress('coordinator'), to: userAddress('u2'), channel: 'coordinator',
      payload: { text: 'y' },
    });
    state = chatReducer(state, { type: 'message', env: envB, convKey: B, active: false });
    state = chatReducer(state, { type: 'read-channel', convKey: A });
