...
export class FakeTransport implements AppTransport {
  ...
  async connect(): Promise<void> { this.connected = true; this.setStatus('open'); }
  ...
  grant(g: Envelope<'pair.grant'>): void { for (const h of this.grantHandlers) h(g); this.setStatus('open'); }
