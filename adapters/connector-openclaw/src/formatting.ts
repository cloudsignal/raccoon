...
// string[] where EACH element maps to exactly one OAM `msg` envelope. The Task 4
...
// Native image envelopes (a dedicated OAM msg type with a media payload) are a
// future OAM protocol extension; when that lands, the outbound adapter (Task 4)
 ...
 * OAM has no hard wire limit — this is a Raccoon-side policy choice.
 */
 ...
 * can render them as markdown links. Native image envelopes (a future OAM
 ...
 *
 * Callers MUST map each returned element to exactly one OAM `msg` envelope,
...
export interface FormatReplyParams {
   ...
   * v1: text-append path only. Native OAM image envelopes are a future
...
/**
 * Format one agent reply into an ordered `string[]` of OAM msg envelope bodies.
 *
 * Steps:
 ...
 *
 * The caller (Task 4 outbound adapter) maps result[i] → the (i+1)-th OAM msg
